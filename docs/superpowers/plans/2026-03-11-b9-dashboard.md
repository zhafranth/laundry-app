# B9 Dashboard Backend — Implementation Plan

> **For agentic workers:** REQUIRED: Use superpowers:subagent-driven-development (if subagents available) or superpowers:executing-plans to implement this plan. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Build the Dashboard backend module with 2 endpoints that serve aggregated stats for the frontend Dashboard and Overview pages.

**Architecture:** Single NestJS module (`DashboardModule`) with 1 controller + 1 service. Reuses existing guards (JwtAuthGuard, RolesGuard, OutletAccessGuard from finance module). All data aggregated from existing tables via Prisma queries — no new migrations needed.

**Tech Stack:** NestJS 11, Prisma 7, PostgreSQL, TypeScript
...
backend/src/dashboard/
  ...
  dashboard.controller.ts    — 2 endpoints (GET dashboard, GET overview)
  dashboard.service.ts       — All aggregation logic
```

No new guards or DTOs needed — both endpoints are simple GETs using existing infrastructure.

---

## Chunk 1: Dashboard Module

### Task 1: Create DashboardModule scaffold
...
- [ ] **Step 1: Create `dashboard.service.ts` with stub methods**
...
> **Note:** Controller uses `@Controller()` (no prefix) with full path in each `@Get()` to avoid route conflict — `outlets/overview` must not be caught by `outlets/:id` pattern. The `overview` endpoint does NOT use OutletAccessGuard since it's not scoped to a single outlet.
...
Add `DashboardModule` import to `backend/src/app.module.ts`:
...
git commit -m "feat(dashboard): scaffold dashboard module with controller and service stubs"
```

---

### Task 2: Implement `getStats()` — Single Outlet Dashboard

**Files:**
- Modify: `backend/src/dashboard/dashboard.service.ts`

This is the main endpoint. It aggregates: today's revenue (+ change %), active orders, approaching deadline count, today's expenses, monthly customers, 7-day revenue chart, recent 5 orders, and Pro-only features (health score, insights, alerts).

- [ ] **Step 1: Add helper — check if outlet has Pro plan**
...
- [ ] **Step 2: Add helper — get today's date range**
...
- [ ] **Step 3: Implement `getStats()` — core metrics**

Replace the stub `getStats` method:
...
async getStats(outletId: string) {
  ...
  const todayRevenue = Number((todayRevenueAgg._sum.totalAmount ?? 0).toString());
  const yesterdayRevenue = Number((yesterdayRevenueAgg._sum.totalAmount ?? 0).toString());
...
- [ ] **Step 4: Implement `getRecentOrders()` helper**
...
- [ ] **Step 5: Implement `getRevenueChart()` helper**
...
- [ ] **Step 6: Implement `getProFeatures()` helper**
...
private async getProFeatures(outletId: string) {
  ...
  const lowStockItems = await this.prisma.inventoryItem.findMany({
    where: {
      outletId,
      isActive: true,
      currentStock: { lte: this.prisma.inventoryItem.fields?.minStockAlert as any },
    },
    select: { name: true, currentStock: true, unit: true },
  });

  // Prisma can't do column comparison directly, fetch and filter in-memory
  ...
  const criticalItems = allItems.filter(
    (item) => Number(item.currentStock.toString()) <= Number(item.minStockAlert.toString()),
  );
  ...
  const insights: { id: string; text: string; type: 'success' | 'warning' | 'info' }[] = [];

  if (latestScore?.insights) {
    const rawInsights = latestScore.insights as Array<{ text: string; type: string }>;
...
git commit -m "feat(dashboard): implement getStats with core metrics, charts, and Pro features"
```

---

### Task 3: Implement `getOverview()` — Multi-Outlet Overview

**Files:**
- Modify: `backend/src/dashboard/dashboard.service.ts`

- [ ] **Step 1: Implement `getOverview()` method**
...
```bash
git add backend/src/dashboard/dashboard.service.ts
git commit -m "feat(dashboard): implement getOverview for multi-outlet stats"
```

---

### Task 4: Final cleanup & add imports

**Files:**
- Verify: `backend/src/dashboard/dashboard.service.ts` — ensure all imports are correct

- [ ] **Step 1: Verify final `dashboard.service.ts` imports**
...
No other imports needed — all types are inferred from Prisma.
...
- [ ] **Step 3: Verify routes in Swagger**

Run: `cd backend && yarn start:dev`
Check: `http://localhost:3000/api/docs` — confirm both endpoints appear under "Dashboard" tag:
- `GET /api/outlets/{id}/dashboard`
- `GET /api/outlets/overview`

- [ ] **Step 4: Final commit**

```bash
git add -A
git commit -m "feat(dashboard): complete B9 dashboard module"
```
