...
export function OrdersPage() {
  ...
  const { mutate: markPaid, isPending: isMarkPaidPending } = useMutation({
    mutationFn: ({ orderId, method, amount }: { orderId: string; method: PaymentMethod; amount: number }) =>
      orderService.markPaid(activeOutletId!, orderId, amount, method),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: queryKeys.orders.list(activeOutletId ?? "", {}) });
      setMarkPaidOrder(null);
    },
  });

  const { mutate: deleteOrd, isPending: isDeletePending } = useMutation({
    mutationFn: (orderId: string) => orderService.delete(activeOutletId!, orderId),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: queryKeys.orders.list(activeOutletId ?? "", {}) });
      setDeleteOrder(null);
    },
  ...
  return (
    <div className="flex flex-col gap-5 pb-8">
      ...
      {markPaidOrder && (
        <MarkPaidModal
          order={markPaidOrder}
          onConfirm={(method, amount) => markPaid({ orderId: markPaidOrder.id, method, amount })}
