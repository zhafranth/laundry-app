  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center" style={{ background: "rgba(11,29,53,0.5)" }} onClick={onClose}>
      <div className="rounded-2xl p-6 w-full" style={{ background: "white", maxWidth: 380, boxShadow: "0 16px 48px rgba(11,29,53,0.18)", animation: "fade-up 0.2s ease" }} onClick={(e) => e.stopPropagation()}>
        <p style={{ fontFamily: "Manrope, system-ui", fontWeight: 800, fontSize: "1rem", color: "#0B1D35", marginBottom: 8 }}>Hapus Pelanggan?</p>
        <p style={{ fontFamily: "Nunito Sans, system-ui", fontSize: "0.8125rem", color: "#5A6B80", marginBottom: 20 }}>
          <strong>{customer.name}</strong> ({customer.phone}) akan dihapus permanen. Riwayat order tetap tersimpan. Tindakan ini tidak bisa dibatalkan.
        </p>
...
export function CustomersPage() {
  ...
  const { mutate: createCustomer, isPending: isCreating } = useMutation({
    mutationFn: (values: CustomerFormValues) => customerService.create(activeOutletId!, values),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["customers", activeOutletId] });
      setShowForm(false);
    },
  });
  ...
  const { mutate: deleteCustomer, isPending: isDeleting } = useMutation({
    mutationFn: (id: string) => customerService.delete(activeOutletId!, id),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["customers", activeOutletId] });
    ...
    <div className="flex flex-col gap-5 pb-8">
      ...
      <div className="flex items-center justify-between">
        <div>
          <div className="flex items-center gap-3">
            ...
            <h1 style={{ fontFamily: "Manrope, system-ui", fontWeight: 800, fontSize: "1.375rem", color: "#0B1D35", margin: 0 }}>Pelanggan</h1>
          </div>
          {!isLoading && (
      ...
      {(showForm || editTarget) && (
        <CustomerFormModal
          ...
          defaultValues={editTarget ? { name: editTarget.name, phone: editTarget.phone, address: editTarget.address ?? "", notes: editTarget.notes ?? "" } : undefined}
      ...
      {/* ── Delete Modal ── */}
      {deleteTarget && (
        <DeleteModal customer={deleteTarget} onConfirm={() => deleteCustomer(deleteTarget.id)} onClose={() => setDeleteTarget(null)} isPending={isDeleting} />
      )}
    </div>
  );
}
